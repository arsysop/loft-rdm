...
public final class CppIntegrityTest extends CxxIntegrityTest {
            
    @Test
    public void directContents() {
        assertEquals(9, sections().stream() //
                .filter(section -> section.getId().equals("cpp")) //$NON-NLS-1$
    ...
    @Test
    public void cppReplaceContents() {
        assertEquals(5, sections().stream() //
                .filter(section -> section.getId().equals("cpp.replace")) //$NON-NLS-1$
                .flatMap(section -> section.getContents().stream()) //
                .filter(Section.class::isInstance) //
                .count());
    }
