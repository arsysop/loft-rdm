...
public final class ExceptIntegrityTest extends CxxIntegrityTest {
            
    @Test
    public void directContents() {
        assertEquals(5, sections().stream() //
                .filter(section -> section.getId().equals("except")) //$NON-NLS-1$
    ...
    @Test
    public void exceptSpecialContents() {
        assertEquals(3, sections().stream() //
                .filter(section -> section.getId().equals("except.special")) //$NON-NLS-1$
                .flatMap(section -> section.getContents().stream()) //
                .filter(Section.class::isInstance) //
                .count());
    }
