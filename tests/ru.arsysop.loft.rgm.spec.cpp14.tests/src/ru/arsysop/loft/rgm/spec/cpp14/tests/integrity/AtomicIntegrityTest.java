...
public final class AtomicIntegrityTest extends CxxIntegrityTest {
            
    @Test
    public void directContents() {
        assertEquals(8, sections().stream() //
                .filter(section -> section.getId().equals("atomics")) //$NON-NLS-1$
    ...
    @Test
    public void atomicsTypesOperationsContents() {
        assertEquals(5, sections().stream() //
                .filter(section -> section.getId().equals("atomics.types.operations")) //$NON-NLS-1$
                .flatMap(section -> section.getContents().stream()) //
                .filter(Section.class::isInstance) //
                .count());
    }
