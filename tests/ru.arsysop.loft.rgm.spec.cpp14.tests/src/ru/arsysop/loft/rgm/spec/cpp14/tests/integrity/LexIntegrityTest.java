...
public final class LexIntegrityTest extends CxxIntegrityTest {
            
    @Test
    public void directContents() {
        assertEquals(14, sections().stream() //
                .filter(section -> section.getId().equals("lex")) //$NON-NLS-1$
    ...
    @Test
    public void lexLiteralContents() {
        assertEquals(8, sections().stream() //
                .filter(section -> section.getId().equals("lex.literal")) //$NON-NLS-1$
                .flatMap(section -> section.getContents().stream()) //
                .filter(Section.class::isInstance) //
                .count());
    }
